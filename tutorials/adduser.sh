...
if [ -f /root/Temp/ptmp ] ; then
  ...
  cp /root/Temp/passwd /root/Temp/ptmp
  cp /root/Temp/shadow /root/Temp/stmp
  cp /root/Temp/group /root/Temp/gtmp
  chmod 700 /root/Temp/stmp
fi
...
chkusr=`cat /root/Temp/ptmp | cut -d ":" -f 1 | grep -c $name`
if [ $chkusr -ne 0 ] ; then
  echo " "
...
chkid=`cat /root/Temp/ptmp | cut -d ":" -f 3 | grep -c $uid`
if [ $chkid -ne 0 ] ; then
  echo " "
...
echo "$tmpgrp2">/root/Temp/group
echo "${tmpgrp1},${name}">>/root/Temp/group

userinfo=$name":x:"$uid":"$gid":"$descri":"$homedir":"$shell
#echo $userinfo
echo $userinfo>>/root/Temp/passwd
